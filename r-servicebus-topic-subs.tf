resource "azurerm_servicebus_subscription" "topic_sub" {
  ...
  default_message_ttl       = each.value.sub_conf.default_message_ttl != null ? format("PT%sM", each.value.sub_conf.default_message_ttl) : null
  lock_duration             = each.value.sub_conf.lock_duration != null ? format("PT%sM", each.value.sub_conf.lock_duration) : null
