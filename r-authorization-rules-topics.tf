resource "azurerm_servicebus_topic_authorization_rule" "listen" {
  ...
  name     = try(format("%s-listen", each.value.custom_name), var.use_caf_naming ? azurecaf_name.servicebus_topic_auth_rule[each.key].result : "listen-default")
...
resource "azurerm_servicebus_topic_authorization_rule" "send" {
  ...
  name     = try(format("%s-listen", each.value.custom_name), var.use_caf_naming ? azurecaf_name.servicebus_topic_auth_rule[each.key].result : "send-default")
  topic_id = azurerm_servicebus_topic.topic[each.value.topic].id
...
resource "azurerm_servicebus_topic_authorization_rule" "manage" {
  ...
  name     = try(format("%s-listen", each.value.custom_name), var.use_caf_naming ? azurecaf_name.servicebus_topic_auth_rule[each.key].result : "manage-default")
  topic_id = azurerm_servicebus_topic.topic[each.value.topic].id
